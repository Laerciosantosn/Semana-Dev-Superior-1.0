...
const CreateRecord: React.FC = () => {
  ...
  const handleChangePlatform = (selectedPlatfom: GamePlatform) => {
    setPlatform(selectedPlatfom);
    const gamesByPlatform = allGames.filter(
      game => game.platform === selectedPlatfom,
    );
  ...
  const handleSubmit = () => {
    ...
    axios
      ...
      .catch(() => Alert.alert('Erro ao cadastra os dados'));
  };
  ...
  useEffect(() => {
    axios
      ...
      .catch(() => Alert.alert('Erro ao listar os ogosj'));
  }, []);
